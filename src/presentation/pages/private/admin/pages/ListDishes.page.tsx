...
const {
  ADMIN,
  admin: { ADD_DISH, EDIT_DISH, VIEW_DISH },
} = PrivateRoutes;
      ...
      <div className="m-5 rounded-md border-2 dark:border-slate-700 md:m-10">
        ...
        >
          ...
          <Column
            field="price"
            header="Price"
            body={(dish: DishModel) => formatCurrency(dish.price)}
            sortable
          />
          <Column
            field="createdAt"
            header="Created At"
            body={(dish: DishModel) => convertDateToShortString(dish.createdAt)}
            sortable
          />
          <Column
            field="updatedAt"
            header="Updated At"
            body={(dish: DishModel) => convertDateToShortString(dish.updatedAt)}
            sortable
          />
            ...
            body={(dish: DishModel) => (
              <div className="flex items-center justify-evenly gap-x-4">
                ...
                <Button
                  ...
                  rounded
                  outlined
                  onClick={() => navigate(`${ADMIN}/${VIEW_DISH(dish.id)}`)}
                />
                <Button
                  unstyled
                  icon="pi pi-trash"
